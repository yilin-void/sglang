...
class W4AFp8Config(QuantizationConfig):
    ...
    def from_config(cls, config: Dict[str, Any]) -> W4AFp8Config:
        ...
        # PATCH: honor the checkpoint's skip list + add fused parents (in_proj_ba
        # from Qwen3.5's in_proj_a/in_proj_b) so bf16 non-128 linear layers are
        # skipped instead of wrongly sent to block-FP8.
        ignored_layers = (
            config.get("ignored_layers")
            or config.get("modules_to_not_convert")
            or []
        )
        _fused_parent = {"in_proj_a": "in_proj_ba", "in_proj_b": "in_proj_ba"}
        _expanded = list(ignored_layers)
        for _n in list(ignored_layers):
            if "." in _n:
                _head, _last = _n.rsplit(".", 1)
                if _last in _fused_parent:
                    _p = f"{_head}.{_fused_parent[_last]}"
                    if _p not in _expanded:
                        _expanded.append(_p)
        ignored_layers = _expanded
        return cls(
...
class W4AFp8MoEMethod(FusedMoEMethodBase):
    ...
    def _apply_mxfp4_fp8(
        ...
        # Keep profile_ids=None: outside flashinfer.autotuner.autotune(True) this
        # selects a cached tactic when available and otherwise its fallback tactic.
        ...
        # SGLANG_MXFP4_AUTOTUNE=1 -> one-time autotune of the humming tactic at the
        # runtime shape. PR #3738's perf curves are autotuned; the default serving
        # path otherwise runs the (much slower) fallback tactic. Tuning the first
        # forward (during warmup) caches the best tactic for later requests.
        if os.environ.get("SGLANG_MXFP4_AUTOTUNE", "0") == "1" and not getattr(
            type(self), "_humming_tuned", False
        ):
            try:
                from flashinfer.autotuner import autotune as _fi_autotune

                with _fi_autotune(tune_mode=True):
                    flashinfer_cutlass_fused_moe(**_fused_moe_kwargs)
                type(self)._humming_tuned = True
                logger.warning(
                    "MXFP4 humming: one-time flashinfer autotune done "
                    "(SGLANG_MXFP4_AUTOTUNE=1)."
                )
            except Exception as _e:  # noqa: BLE001
                logger.warning(
                    "MXFP4 humming autotune failed (%s); using fallback tactic.", _e
                )
        flashinfer_cutlass_fused_moe(**_fused_moe_kwargs)
        return StandardCombineInput(hidden_states=output)
